...
<div class="flex h-screen overflow-hidden">
	...
	<aside class="flex w-52 shrink-0 flex-col border-r border-border-subtle bg-bg-surface">
		<div class="px-5 pt-5 pb-4">
			<h1 class="font-display text-[19px] font-bold tracking-tight text-text-primary">I Dropped the LLM</h1>
			<p class="mt-1 text-[12px] leading-snug text-text-tertiary">48-block residual network<br/>97 shuffled linear layers</p>
		</div>

		<nav class="flex flex-1 flex-col gap-0.5 px-2">
			{#each tabs as tab}
				<button
					class="nav-item flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-left font-display text-[15px] font-medium cursor-pointer
						{activeTab === tab.id
							? 'bg-bg-card-hover text-text-primary nav-item-active'
							: 'text-text-secondary hover:bg-bg-card hover:text-text-primary'}"
					onclick={() => { activeTab = tab.id; }}
				>
					<span class="text-[13px] {activeTab === tab.id ? phases[tab.id].textClass : 'opacity-40'}">{phases[tab.id].icon}</span>
					{tab.label}
					{#if tab.hasData}
		...
		<div class="border-t border-border-subtle px-4 py-3">
			<div class="flex items-center gap-2">
				...
				<p class="text-[12px] leading-snug text-text-tertiary">
